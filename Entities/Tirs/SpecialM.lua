...
function ent:update(dt)
   ...
   if (distance < (self.image:getWidth() / 2 + Tank.BaseImage:getWidth() / 2) * Reso.Scale * 0.2) then
...
--- Affiche l'entite.
function ent:draw()
   love.graphics.draw(self.image, self.x, self.y, self.angle, Reso.Scale * 0.2, Reso.Scale * 0.2, self.image:getWidth() / 2, self.image:getHeight() / 2)
end

--- Code a executer avant la destruction de l'entite.
function ent:Die()
   print("SpecialM " .. self.id .. " detruit.")
end

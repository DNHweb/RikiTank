...
function ent:load( x, y )
   self:setPos( x, y )
   self.image = picResistance
   self.stime = love.timer.getTime()
end

--- Mise-a-jour de l'entite.
-- Calcule de la distance, si collision avec le tank alors le joueur voit sa resistance augmentee.
...
function	ent:update(dt)
   ...
   if distance < (self.image:getWidth() / 2 + Tank.BaseImage:getWidth() / 2) * Reso.Scale then
      Tank.Dammage = Tank.Dammage - (Tank.Dammage * 0.20)
